...
```bash
cd services/api
/opt/homebrew/bin/python3.12 -m venv .venv && . .venv/bin/activate   # Mac；其他平台用系统 python3.12
pip install -e ".[dev]"
cp .env.example .env
# 编辑 .env（JWT、数据库等）
...
学生端 web SPA 本地开发（`http://127.0.0.1:1420`）或 Vite dev 可能使用不同端口（1420、5173 等）。默认开启 **`CORS_DEV_LOCALHOST_REGEX=true`**（可在 `.env` 覆盖）：对 `http(s)://localhost`、`127.0.0.1`、`tauri.localhost` **任意端口**允许跨域。生产环境应将 `CORS_DEV_LOCALHOST_REGEX=false` 并仅保留站点域名。
...
Docker 与生产部署配置见 `deploy/` 目录（Nginx、systemd、部署检查清单）。
